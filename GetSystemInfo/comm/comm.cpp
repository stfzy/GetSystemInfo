...
{ 
	...
	PROCESS_MEMORY_COUNTERS_EX promemcou;
	GetProcessMemoryInfo(hProcess,(PROCESS_MEMORY_COUNTERS*)&promemcou,sizeof(promemcou)); 
 ...
 LONGLONG file_time_2_utc(const FILETIME* ftime)
{
 ...
 BOOL WMI_DoWithHarddiskSerialNumber( WCHAR *SerialNumber,UINT uSize)  
 {  
	 UINT    iLen;  
	 UINT    i;  
...
bool GetUserInfo(USERINFO& Info)
{
 ...
 //MessageBoxA(0,"message 1 " ,0,0);
    ...
    while (pEnumerator)
    {
		...
		ZeroMemory(&vtProp2,sizeof(vtProp2));
		WCHAR szTmp[MAX_PATH]={0};
		hr = pclsObj->Get(L"Caption", 0, &vtProp, 0, 0);  //video desc 
        hr = pclsObj->Get(L"AdapterRAM",0,&vtProp2,0,0);
		ULONGLONG ull1 = (vtProp2.ullVal)/1024/1024;
		
		wsprintfW(szTmp,L"%s (%d MB)",vtProp.bstrVal,ull1);
	...
	while (pEnumerator)
	{
		...
		wsprintfW(diskInfo.szInfo,L"%s(%d GB)",vtProp1.bstrVal,_wtoi64(vtProp.bstrVal)/1024/1024/1024);
		BSTR bTmp = vtProp2.bstrVal;
		
		if( bTmp != NULL && (int)bTmp != 0xcccccccc)
		{
			WMI_DoWithHarddiskSerialNumber(bTmp,40);
			wcscat_s(diskInfo.szInfo,MAX_PATH,L"    序列号：");
			do 
			{
				if(*bTmp == L' ')
					bTmp++;
	...
	while (pEnumerator)
	{
		...
		if(vtProp1.vt != VT_EMPTY && vtProp1.vt != VT_NULL)
		{
			wcscat_s(wTmpMem,MAX_PATH,vtProp1.bstrVal);
			wcscat_s(wTmpMem,MAX_PATH,L" ");
		}
		if(vtProp2.vt != VT_EMPTY && vtProp2.vt != VT_NULL)
		{
			WCHAR wSpeed[100];
			_itow_s(vtProp2.iVal,wSpeed,10);
			wcscat_s(wTmpMem,MAX_PATH,wSpeed);
			wcscat_s(wTmpMem,MAX_PATH,L" MHz ");
		}
		if(vtProp.vt != VT_EMPTY && vtProp.vt != VT_NULL)
		{
			WCHAR wDx[100];
			_itow_s(_wtoi64(vtProp.bstrVal)/1024/1024,wDx,10);
			wcscat_s(wTmpMem,MAX_PATH,wDx);
			wcscat_s(wTmpMem,MAX_PATH,L" MB");
		}
	...
	while (pEnumerator)
	{
		...
		if (buf)
		{
			...
			if(pIn == NULL|| (int)buf == 0xcccccccc)
...
{ 
	...
	{
		if(GetLocalMachineRegInfo(TEXT("SOFTWARE\\Microsoft\\Internet Explorer"),L"Version",pIeVersion,50))
		{
			return TRUE;
		}
